...
const Dashboard = () => {
   ...
   * --------------------------------------------------------------------------
   * Scrolls to bottom whenever messages change OR loading begins/ends.
   * During live streaming this runs on EVERY incoming chunk, so we use
   * behavior: "auto" — keep-up-with-streaming is better than queued smooth
   * scrolls (smooth would stutter while many tokens arrive per second).
   */
  useEffect(() => {
                  ...
                  >
                    {isUser ? (
                      ...
                      /* AI ANSWER (Rendered with ReactMarkdown custom components)
                         NOTE: While the AI is streaming, `msg.streaming` is true
                         and `msg.content` grows on every "ai_typing" chunk.
                         ReactMarkdown re-parses the growing text — giving the
                         live word-by-word typing effect. */
